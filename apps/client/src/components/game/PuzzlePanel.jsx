...
// Keep track of visited levels to avoid replaying animations
const visitedLevels = new Set();

const PuzzlePanel = ({ level, showingStoryFragment, storyFragmentText }) => {
	...
	const buildLevelMarkdown = () => {
		...
		if (level.transmission?.type === "image" && level.transmission?.imageUrl) {
		...
		if (level.hintPoem) {
			const hintText = Array.isArray(level.hintPoem) ? level.hintPoem.join(' ') : level.hintPoem;
			markdown += `## Hint\n\n> "${hintText}"\n\n`;
		}
	...
	const { displayedText: typedCommand, isComplete: commandComplete } = useTypingEffect(
		command,
		30,
	...
	const { displayedText: typedContent, isComplete: contentComplete } = useTypingEffect(
		levelMarkdown,
		15,
		fetchComplete && !isVisited
	);

	const storyCommand = `> /sysctl -dmsg --priority=high`;
	const { displayedText: typedStoryCommand, isComplete: storyCommandComplete } = useTypingEffect(
		storyCommand,
		30,
		showingStoryFragment
	);
	const { displayedText: typedStoryFragment, isComplete: storyComplete } = useTypingEffect(
		storyFragmentText || "",
		20,
	...
	}, [typedCommand, typedContent, typedStoryFragment, dots]);
		...
		lines.forEach((line, idx) => {
			if (line.startsWith('```')) {
				if (!inCodeBlock) {
					...
					elements.push(
						<div key={`codeblock-${codeBlockStart}`} className="group relative my-4 border border-gray-700 bg-[#0a0a0a] transition-all duration-300 animate-fadeIn hover:border-green-500/30">
							<div className="flex items-center justify-between border-b border-gray-800 bg-black/50 px-3 py-1">
								<span className="text-[10px] text-gray-500">ENCRYPTED DATA BLOCK</span>
								<button 
									onClick={() => navigator.clipboard.writeText(codeBlockContent.join('\n'))}
									className="hidden text-[10px] text-green-500 hover:text-green-300 group-hover:block"
								>
									[COPY]
								</button>
							</div>
			...
			if (line.startsWith('# ')) {
				elements.push(
					<div key={idx} className="mb-4 border-b border-cyan-900/30 pb-2">
						<div className="text-lg font-bold text-cyan-400">{line.substring(2)}</div>
					</div>
				);
				...
				elements.push(
					<div key={idx} className="mt-6 mb-2 flex items-center gap-2">
						...
						<div className="text-sm font-bold text-green-400">{line.substring(3)}</div>
					</div>
				);
			} else if (line.startsWith('> ')) {
				// hint as quoteblock
				const hintText = line.substring(2).replace(/^"/, '').replace(/"$/, '');
				elements.push(
					<div key={idx} className="relative my-4 border-l-2 border-yellow-600/50 bg-yellow-950/10 p-4 transition-all duration-300 animate-fadeIn">
						<div className="absolute top-2 left-2 text-2xl text-yellow-700/30">"</div>
						<div className="px-4 text-center text-sm italic text-yellow-200/70">
							{hintText}
						</div>
						<div className="absolute right-2 bottom-2 text-2xl text-yellow-700/30">"</div>
					</div>
				);
			} else if (line.startsWith('![')) {
				// image parser
				...
				if (match) {
					elements.push(
						<div key={idx} className="my-4 border border-gray-800 bg-black p-1">
							<img 
								src={match[2]} 
								alt={match[1]}
								className="crt-glow h-auto max-w-full transition-all duration-500 animate-fadeIn"
								style={{ maxHeight: "300px", imageRendering: "pixelated" }}
							/>
							<div className="mt-1 text-center text-[9px] text-gray-600">{match[1]}</div>
						</div>
					);
				}
			} else if (line.startsWith('*') && line.endsWith('*')) {
				elements.push(<div key={idx} className="text-xs italic text-yellow-600/80 my-2 text-center">{line.slice(1, -1)}</div>);
			} else if (line.trim() === '') {
				elements.push(<div key={idx} className="h-2"></div>);
			} else {
				elements.push(<div key={idx} className="text-xs text-gray-300 leading-relaxed">{line}</div>);
			}
		});
		...
		return (
			<div className="puzzle-panel flex h-full flex-col border border-gray-800 bg-black font-mono">
				<div className="flex shrink-0 items-center justify-between border-b border-gray-800 bg-[#0a0a0a] px-3 py-2">
					...
					<div className="text-[9px] text-gray-600">SECURE CHANNEL</div>
				</div>
				<div ref={scrollRef} className="scrollbar-thin min-h-0 flex-1 overflow-y-auto p-4">
					<div className="space-y-2">
						{/* Command line */}
						<div className="text-xs">
							<span className="text-green-400">{typedStoryCommand}</span>
							{!storyCommandComplete && <span className="animate-pulse">▊</span>}
						</div>
						{/* Story content */}
						{storyCommandComplete && (
							<div className="mt-4 border border-cyan-900/50 bg-cyan-950/10 p-4 shadow-[0_0_20px_rgba(8,145,178,0.1)]">
								...
								<p className="font-mono text-sm leading-relaxed text-cyan-300">
									{typedStoryFragment}
									{!storyComplete && <span className="animate-pulse">▊</span>}
								</p>
							</div>
						)}
					</div>
	...
	return (
		<div className="puzzle-panel flex h-full flex-col border border-gray-800 bg-black font-mono">
			...
			<div ref={scrollRef} className="scrollbar-thin min-h-0 flex-1 overflow-y-auto p-4 bg-black">
				<div className="space-y-2">
					{/* Command line */}
					<div className="text-xs">
						<span className="text-green-400">{typedCommand}</span>
						{!commandComplete && <span className="animate-pulse">▊</span>}
					</div>
					{/* Fetching status */}
					...
					{fetchComplete && (
						<div className="mt-3 pt-3 border-t border-gray-800/50">
							...
							{!contentComplete && <span className="animate-pulse text-green-400">▊</span>}
						</div>
					)}
				</div>
			</div>
